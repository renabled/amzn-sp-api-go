...
type TransportLabel struct {
	...
	// Format: date-time
	LabelCreateDateTime strfmt.DateTime `json:"labelCreateDateTime,omitempty"`
...
func (m *TransportLabel) Validate(formats strfmt.Registry) error {
	...
	if err := m.validateLabelCreateDateTime(formats); err != nil {
		res = append(res, err)
	}
...
func (m *TransportLabel) validateLabelCreateDateTime(formats strfmt.Registry) error {
	if swag.IsZero(m.LabelCreateDateTime) { // not required
		return nil
	}

	if err := validate.FormatOf("labelCreateDateTime", "body", "date-time", m.LabelCreateDateTime.String(), formats); err != nil {
		return err
	}
