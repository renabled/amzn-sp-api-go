...
func (a *Client) CreateUploadDestinationForResource(params *CreateUploadDestinationForResourceParams, opts ...ClientOption) (*CreateUploadDestinationForResourceCreated, error) {
	...
	if params == nil {
		params = NewCreateUploadDestinationForResourceParams()
	}
	op := &runtime.ClientOperation{
		ID:                 "createUploadDestinationForResource",
		Method:             "POST",
		PathPattern:        "/uploads/2020-11-01/uploadDestinations/{resource}",
		ProducesMediaTypes: []string{"application/json"},
		ConsumesMediaTypes: []string{"application/json"},
		Schemes:            []string{"https"},
		Params:             params,
		Reader:             &CreateUploadDestinationForResourceReader{formats: a.formats},
	...
	success, ok := result.(*CreateUploadDestinationForResourceCreated)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for createUploadDestinationForResource: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
